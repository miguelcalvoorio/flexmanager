...
export class CustomHttp extends Http { 
    ...
    private handleError(error: any) { 
        if (error.status === 401) { 
            ...
            // I do not why, but this does not work --> this.router.navigate(['/login']);
        } 

        return Observable.throw({'status': error.status, 'body': error._body}); 
    } 
} 

export function customHttpFactory(xhrBackend: XHRBackend, requestOptions: RequestOptions, router: Router): Http { 
    return new CustomHttp(xhrBackend, requestOptions, router); 
} 
